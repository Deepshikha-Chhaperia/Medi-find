  ...
  "verifications": [
    ...
    for r in results:
        if r.facility_id in v_map:
            ...
            r.match_confidence = "Verified High" if v.get("is_real_match") else "Partial Match"
...
def run_query(request: SearchRequest) -> SearchResponse:
    ...
    with get_db() as conn:
        ...
        for rank_idx, (fid, chunk) in enumerate(
            ...
            fac = fetchone(conn, "SELECT * FROM facilities WHERE facility_id = ?", (fid,))
            if not fac:
                continue

            cap_ids = []
            try:
                caps = fetchall(conn, "SELECT capability_id FROM facility_capabilities WHERE facility_id = ?", (fid,))
            ...
            if fac.get("source_doc_id"):
                try:
                    doc = fetchone(conn, "SELECT source_file FROM documents WHERE doc_id = ?", (fac["source_doc_id"],))
